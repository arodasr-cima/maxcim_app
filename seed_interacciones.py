...
Cada fila copia el `id_periodo` de su material (o el periodo vigente en la
fecha simulada, para una conversación libre) -igual que hace
`registrar_interaccion` en producción-, así que lo sembrado aparece de
entrada en /aulas/<aula>/avance y en el detalle del alumno sin tener que
elegir "Todos los periodos": esas vistas arrancan acotadas al periodo
vigente hoy y, dentro de este, al primer tema de la docente.
...
Uso:
    ...
    python seed_interacciones.py --alumno 79398411 --material 1 --material 2 --n 20
    ...
    # Repetir con otro --alumno (y opcionalmente --seed distinto) para llenar
    # varias filas de la tabla "Resultados por material" del aula:
    python seed_interacciones.py --alumno 79398411 --seed 1
    python seed_interacciones.py --alumno 88123456 --seed 2 --correct-rate 0.4
...
IMPORTANTE:
- El alumno solo aparece en /aulas/<aula>/avance y en su detalle si su ID
  coincide con un alumno del roster que devuelve la API de CIMA. Un ID
  inventado igual cuenta en las tarjetas del tablón y en GET /api/interacciones,
  pero no en las pantallas por aula.
- El material usado debe tener ya `id_periodo`/`id_tema` asignados (lo trae
  cualquier material creado desde la consola, que lo exige al guardar); si
  usas uno viejo sin esos campos, sus interacciones quedan con `id_periodo`
  NULL y solo se ven eligiendo "Todos los periodos" en el filtro.
"""
...
from models import TIPO_ORACION, TIPO_ORACION_IMAGEN, Interaccion, Material
from services.periodos import periodo_for_date
...
# Tipos de material cuyo contenido es una lista de oraciones (texto plano
# para leer en voz alta); una `oracion_imagen` guarda además, por oración,
# los dos sustantivos e imágenes, pero para simular una interacción de
# lectura alcanza con el texto de la oración.
SENTENCE_MATERIAL_TYPES = (TIPO_ORACION, TIPO_ORACION_IMAGEN)


def load_material_items(material: Material) -> list[dict]:
    """Devuelve una lista de 'items' de práctica del material: preguntas de
    un cuento, o el texto de cada oración de una `oracion`/`oracion_imagen`."""
    ...
    if material.tipo_material in SENTENCE_MATERIAL_TYPES:
        sentences: list[str] = []
        if path and os.path.exists(path):
            try:
                with open(path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                for entry in data:
                    # `oracion`: lista de strings. `oracion_imagen`: lista de
                    # {"texto": ..., "sustantivos": [...]}; solo hace falta el texto.
                    text = str(entry.get("texto") if isinstance(entry, dict) else entry).strip()
                    if text:
                        sentences.append(text)
            except (OSError, json.JSONDecodeError):
                sentences = []
        if not sentences:
            # Registro antiguo de `oracion`: texto plano en la columna
            # (`oracion_imagen` siempre guarda JSON, nunca cae en este caso).
...
def do_seed(args) -> int:
    ...
    if n_material > 0:
        ...
        for m in materials:
            ...
            if m.id_periodo is None:
                print(f"  (aviso) el material {m.id} '{m.nombre_material}' no tiene periodo asignado; "
                      f"sus interacciones solo se verán eligiendo 'Todos los periodos'.")
        if not pools:
    ...
    for i in range(args.n):
        if i in conversacion_idx:
            ...
            material_id = None
            # Igual que registrar_interaccion en producción: una conversación
            # libre toma el periodo vigente en la fecha simulada, no el de hoy.
            periodo_de_la_fila = periodo_for_date(stamps[i].date())
            id_periodo = periodo_de_la_fila.id if periodo_de_la_fila else None
        else:
            material, items = rng.choice(pools)
            item = rng.choice(items)
            pregunta, respuesta, apreciacion = build_row(item, flags[i], rng)
            material_id = material.id
            # Igual que registrar_interaccion: el periodo se copia del material,
            # no de la fecha de la interacción.
